...
def fetch_all_verses(session, bible_id: str, all_chapters: dict, out_dir: str, start_from: str | None = None):
    """
    Step 4: loop every book → every chapter, fetch chapter content
    (which includes verses) and append results to bible-verses.json.

    If *start_from* is given (a book id like "2PE"), iteration over the
    books skips straight to that book — books before it in all_chapters
    are not touched at all this run.
    """
    ...
    book_ids = list(all_chapters.keys())

    if start_from:
        if start_from in book_ids:
            start_idx = book_ids.index(start_from)
            skipped = book_ids[:start_idx]
            book_ids = book_ids[start_idx:]
            print(f"  --start-from {start_from}: skipping {len(skipped)} earlier book(s)")
        else:
            print(f"  [warn] --start-from book '{start_from}' not found in bible-chapters.json; "
                  f"processing all books instead")

    total_books = len(book_ids)
    for book_idx, book_id in enumerate(book_ids, start=1):
        chapters = all_chapters[book_id]
...
def parse_args():
    ...
    p.add_argument(
        ...
        metavar="BOOK_ID",
        help=(
            "Resume a stalled run from this book id (e.g. 2PE). "
            "When set, steps 1-3 are skipped entirely and bible-chapters.json "
            "is loaded from --out-dir; step 4 then begins at this book."
        ),
    )
    return p.parse_args()
...
def main():
    ...
    if args.start_from:
        chapters_path = os.path.join(out_dir, "bible-chapters.json")
        print(f"\n--start-from {args.start_from}: loading cached chapters from {chapters_path}")
        if not os.path.exists(chapters_path):
            print(f"  [error] {chapters_path} not found — cannot resume without it.")
            sys.exit(1)
        all_chapters = load_json(chapters_path)
    else:
    ...
    if not args.skip_verses:
        fetch_all_verses(session, bible_id, all_chapters, out_dir, start_from=args.start_from)
